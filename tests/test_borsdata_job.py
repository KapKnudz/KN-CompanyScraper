...
def test_job_fails_only_company_with_per_instrument_report_error():
    first = make_company(1, "First")
    broken = make_company(2, "Broken")
    ingestion_service = MagicMock()
    ingestion_service.get_report_bundles.return_value = {
    ...
    result = BorsdataJob(ingestion_service, job_repository).run([first, broken])

    assert result.synced == 1
    assert result.failed == 1
    assert result.failures == (
        "Broken: Börsdata report sync failed for instrument 20: NOT_ACTIVE",
    )
    assert ingestion_service.sync_company.call_count == 1
    ingestion_service.sync_company.assert_called_once_with(
        first, report_bundle=InstrumentReportBundle(10, (), (), ())
    )
