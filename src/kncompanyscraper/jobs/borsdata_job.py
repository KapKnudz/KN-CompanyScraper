...
class BorsdataJob:
    ...
    def run(self, companies: list, *, price_max_count: int | None = None) -> BorsdataJobResult:
        synced = 0
        failures = []
        ...
        for offset in range(0, len(eligible), self.BATCH_SIZE):
            ...
            for company, job_id in batch:
                try:
                    ...
                    if bundle.error:
                        raise ValueError(
                            "Börsdata report sync failed for instrument "
                            f"{company.borsdata_id}: {bundle.error}"
                        )
                    if price_max_count is None:
                        self.ingestion_service.sync_company(
                            company, report_bundle=bundle
                        )
                    else:
                        self.ingestion_service.sync_company(
                            company,
                            report_bundle=bundle,
                            price_max_count=price_max_count,
                        )
                except Exception as exc:
                    error = f"{company.name}: {exc}"
                    failures.append(error)
